...
from anchor import anchor_build
...
from model_builder import post_process_builder
...
def _build_yoloV3_model(model_config,is_training,reuse_weights=tf.AUTO_REUSE):
    ...
    #生成anchors
    anchor_generator = anchor_build.Anchor_Generator(model_config["Anchors"])

    #NMS验证模型时再写
    non_max_suppression_fn,score_converter_fn= post_process_builder.build(config=model_config["post_process"])

    #构建loss函数
    classification_loss, localization_loss,confidence_loss,hard_example_mined= losses_builder.build(model_config)

    return YOLOV3.YOLOV3Meta(istrain=is_training,anchor_generator=anchor_generator,
                             matcher=match,feature_extractor=feature_extractor,
                             box_predictor=BoxPredictor,
                             non_max_suppression_fn=non_max_suppression_fn,
                             score_convert_fn=score_converter_fn,
                             ...
                             resize=model_config["resize"],batch_size=model_config["batch_size"])
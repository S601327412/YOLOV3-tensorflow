...
class YOLOV3Meta(detection_model.Detection_model):
    ...
    def _predictor(self,preprocess_input):
        ...
        with slim.arg_scope([slim.batch_norm],is_training=self._istrain,
            ...
            with tf.variable_scope(self._scope,values=[preprocess_input]):
                feature_map = self._feature_extractor.extractor_feature(preprocess_input,num_anchors=3,num_class=self.num_classes,sess=self._sess,debug=self.nodebug)

        _anchors,self._grid_cell,self._scale_size,per_grid_num_anchor= self._anchor_generator._generator(
    ...
    def _loss(self,predictor_dict):
        with tf.name_scope("Loss",values=[predictor_dict,self._anchors]):
            ...
            for i,matches in enumerate(batch_match):
                ...
                pre_box_list = boxes_utils.decode(self._anchors[i],tf.unstack(predictor_loc_target),self._grid_cell[i],self._resize,scale=self._scale_size[i],sess=self._sess)
    ...
    def _target_assign(self,groundtruth_box,groundtruth_class):
        ...
        return self._matcher.match(groundtruth_box_list,groundtruth_class_with_one_hot,self._anchors,self._resize[0])
    ...
    def preprocess(self,image):
        ...
        return tf.image.resize_images(image,self._resize,method=0,align_corners=True) * (1.0 / 255.0)
